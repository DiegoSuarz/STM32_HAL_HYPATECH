...
int main(void)
{
  ...
  HAL_TIM_Base_Start(&htim2); //inicializar el modo contador
  /* USER CODE END 2 */
  ...
  LCD_STM32_Print_String("PULSOS:");
  while (1)
  {
	  pulsos = __HAL_TIM_GET_COUNTER(&htim2);
	  if(pulsos == 10)
	  {
		  __HAL_TIM_SET_COUNTER(&htim2,0);
	  }
	  sprintf(buffer,"%u",pulsos);
	  LCD_STM32_Set_Cursor(0,Fila2);
	   LCD_STM32_Print_String(buffer);
	   HAL_Delay(200);
    /* USER CODE END WHILE */
